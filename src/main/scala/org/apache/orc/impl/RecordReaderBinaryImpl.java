...
//import org.apache.orc.impl.ColumnDiskRangeList.CreateColumnRangeHelper;
...
//      }
  ...
  public class ColumnDiskRange {
    ...
    public boolean equals(Object other) {
      ...
      if (other instanceof ColumnDiskRange) {
        ...
        return otherColumnDiskRange.offset == this.offset && otherColumnDiskRange.length == this.length;
      }
      return false;
    }
  }
  ...
  private DiskRangeList planReadColumnData(
    ...
    ColumnDiskRangeList.CreateColumnRangeHelper list = new ColumnDiskRangeList.CreateColumnRangeHelper();
    for (OrcProto.Stream stream : streamList) {
      long length = stream.getLength();
      int column = stream.getColumn();
      OrcProto.Stream.Kind streamKind = stream.getKind();
...
//        stripeFooter.getColumnsList(), types, bufferSize, true);
    ...
    bufferChunks = ((RecordReaderBinaryUtils.DefaultDataReader)dataReader).readFileColumnData(toRead, stripe.getOffset(), false);
    System.out.println("bufferChunks.listSize():" + bufferChunks.listSize());
    if (LOG.isDebugEnabled()) {
